...
import { ADMIN, PARTNER, SYSTEM } from 'src/shared/constants';
...
export class PartnerWelcomeComponent implements OnInit {
  ...
  continue(model: UserModel) {
    ...
    this.accountService.getUserByEmail(this.email).subscribe(data => {
      if (data && data.UserId) {
        this.jusLogin = true;
      } else {
        this.jusLogin = false;
  ...
  login(model: UserModel) {
    this.accountService.login({email: model.Email, password: model.Password }).subscribe(user => {
      if (user && user.UserId) {
        this.error = '';
